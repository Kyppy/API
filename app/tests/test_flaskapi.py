...
class TestUsers(unittest.TestCase):
    ...
    def test_delete_empty_record(self):
        response = self.app.delete('/api/v1/red_flag/100')
        result = json.loads(response.data)
        self.assertEqual(result['status'], 404)
   
    def test_post_record_and_get_record(self):
        response = self.app.post('/api/v1/red_flags', data=json.dumps(self.data), content_type='application/json')
    ...
    def test_delete_record(self):
        self.app.post('/api/v1/red_flags', data=json.dumps(self.data), content_type='application/json')
